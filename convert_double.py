...
from project import export_ast

node_list = []
node_right = []


def convert_double(args):
    name = args["name"]
    convert_double.mutability = False
    mutability_checker(name,args)
    if convert_double.mutability:
        mut = " mut "
    else:
        mut = " "
    a = "let" +mut+ args["name"] + ":" + "f64" + " " + "=" + " "
    # print(a)
    b=a
    global_values.declaration_tracker[args["name"]]="double"
            ...
            if type(i) != dict and type(i) != list:
                ...
                if i["_nodetype"] == "Assignment":
                    if i["lvalue"]["name"]==name:
                        convert_double.mutability=True
